...
interface ReviewsListProps { className?: string; }
  ...
  const columns = React.useMemo<ColumnDef<TenantReviewRecord>[]>(() => [
    {
      accessorKey: 'target',
      header: 'Reviewed',
      cell: ({ row }) => <span className="font-semibold text-white">{row.getValue('target')}</span>,
    },
    {
      accessorKey: 'propertyName',
      header: 'Property',
      cell: ({ row }) => <span className="text-blue-200/60 text-sm">{row.getValue('propertyName')}</span>,
    },
    {
      accessorKey: 'rating',
      header: 'Rating',
    ...
    },
    {
      ...
      header: 'Review',
      cell: ({ row }) => (
        <div className="max-w-xs truncate text-blue-200/60 text-sm" title={row.getValue('comment')}>
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div className="flex gap-1">
          <Link href={`/tenant/reviews/${row.original.id}`} className="p-2 text-blue-300/40 hover:text-white hover:bg-white/10 rounded-lg transition-colors inline-flex">
  ...
  if (error) {
    return (
      <div className="bg-red-500/10 border border-red-500/20 rounded-2xl p-10 text-center">
        ...
        <h3 className="text-base font-semibold text-white mb-1">Failed to load reviews</h3>
        <p className="text-blue-200/50 text-sm mb-4">There was an issue fetching your reviews.</p>
      ...
      <div className="flex flex-col sm:flex-row gap-3 items-stretch sm:items-center justify-between bg-white/5 backdrop-blur-sm rounded-2xl p-4 border border-white/10">
        <div className="flex flex-col sm:flex-row gap-3 flex-1">
          ...
          >
            ...
            {[5, 4, 3, 2, 1].map((n) => (
              <option key={n} value={String(n)} className="bg-slate-900">{n} Stars</option>
            ))}
          </select>
        </div>
      ...
      <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl overflow-hidden">
        {isLoading ? (
          ...
          <div className="p-16 text-center">
            ...
            <p className="text-blue-200/40 text-sm mb-5">Your review history will appear here.</p>
            <Link href="/tenant/reviews/new" className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-sm font-semibold transition-colors">
