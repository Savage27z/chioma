...
interface ReviewFormProps { reviewId?: string; }
...
export function ReviewForm({ reviewId }: ReviewFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await updateReviewMutation.mutateAsync({ id: reviewId, payload: { rating, comment } });
  ...
  return (
    <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-3xl p-6 sm:p-8 space-y-6">
      <div>
        ...
        <p className="text-blue-200/50 mt-1">{review ? 'Update your review below.' : 'View your review.'}</p>
      </div>

      <form onSubmit={handleSubmit} className="space-y-5">
        <div>
          <label className="block text-xs font-bold text-blue-200/50 uppercase tracking-wider mb-3">Rating</label>
          <StarRating value={rating} onChange={setRating} size={28} />
        </div>

        <div>
          <label className="block text-xs font-bold text-blue-200/50 uppercase tracking-wider mb-2">Review</label>
          <textarea
            value={comment}
