...
const DocumentViewerModal = dynamic(
  () => import('@/components/documents').then((mod) => ({ default: mod.DocumentViewerModal })),
  { ssr: false },
);
const DocumentUploadModal = dynamic(
  () => import('@/components/documents').then((mod) => ({ default: mod.DocumentUploadModal })),
  { ssr: false },
);
const DocumentListModal = dynamic(
  () => import('@/components/documents').then((mod) => ({ default: mod.DocumentListModal })),
  { ssr: false },
);

const mockDocuments: Document[] = [
  {
    id: '1', name: 'Lease Agreement - 123 Main St.pdf', type: 'pdf',
    url: 'https://www.w3.org/WAI/ER/tests/xhtml/testfiles/resources/pdf/dummy.pdf',
    size: 2457600, uploadedBy: 'user-1', uploadedByName: 'John Landlord',
    uploadedAt: new Date('2024-03-15T10:30:00').toISOString(), category: 'lease',
    description: 'Annual lease agreement for property at 123 Main Street',
  },
  {
    id: '2', name: 'Property Inspection Report.pdf', type: 'pdf',
    url: 'https://www.w3.org/WAI/ER/tests/xhtml/testfiles/resources/pdf/dummy.pdf',
    size: 1843200, uploadedBy: 'user-2', uploadedByName: 'Jane Inspector',
    uploadedAt: new Date('2024-03-10T14:20:00').toISOString(), category: 'inspection',
    description: 'Move-in inspection report with photos',
  },
  {
    id: '3', name: 'Payment Receipt March 2024.pdf', type: 'pdf',
    url: 'https://www.w3.org/WAI/ER/tests/xhtml/testfiles/resources/pdf/dummy.pdf',
    size: 512000, uploadedBy: 'user-3', uploadedByName: 'Bob Tenant',
    uploadedAt: new Date('2024-03-01T09:15:00').toISOString(), category: 'payment',
    description: 'Rent payment receipt for March 2024',
  },
];

const initialDocuments = process.env.NODE_ENV === 'production' ? [] : mockDocuments;
...
export default function TenantDocumentsPage() {
  ...
  const handleUpload = async (files: File[], metadata: DocumentMetadata): Promise<void> => {
  ...
  return (
    <div className="space-y-8">
      ...
      <div>
        ...
        <p className="text-blue-200/50">Upload, view, and manage your rental documents.</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-6">
          ...
          <h3 className="text-2xl font-black text-white mb-0.5">{documents.length}</h3>
        ...
        <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-6">
          ...
          <h3 className="text-2xl font-black text-white mb-0.5">{new Set(documents.map((d) => d.category)).size}</h3>
      ...
      <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl overflow-hidden">
        ...
        {documents.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 text-center px-6">
            ...
            <p className="text-blue-200/40 text-sm mb-6">Your landlord hasn&apos;t uploaded any documents for your account.</p>
            <button
              onClick={() => setIsUploadModalOpen(true)}
      ...
      <DocumentViewerModal document={selectedDocument} onClose={() => setSelectedDocument(null)} onDownload={handleDownload} />
      <DocumentUploadModal isOpen={isUploadModalOpen} onClose={() => setIsUploadModalOpen(false)} onUpload={handleUpload} />
      <DocumentListModal
        documents={documents}
        ...
        onUploadClick={() => { setIsListModalOpen(false); setIsUploadModalOpen(true); }}
      />
    </div>
  );
}
