  ...
  return (
    <div className="space-y-8">
      ...
      <div className="relative group">
        ...
        <div className="relative bg-white/5 backdrop-blur-sm border border-white/10 rounded-3xl p-8 flex flex-col md:flex-row items-center justify-between gap-6 overflow-hidden">
          <div className="flex items-center gap-6 z-10">
            ...
            <div>
              <h1 className="text-3xl font-black tracking-tight text-white mb-1">Stellar Ledger</h1>
              <p className="text-blue-200/50 font-medium">Immutable transaction history powered by Stellar Network.</p>
            </div>
          </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatItem title="Total Volume" value="$128,450" icon={Wallet} color="blue" />
        <StatItem title="Success Rate" value={`${stats.total ? ((stats.success / stats.total) * 100).toFixed(1) : 0}%`} icon={CheckCircle2} color="emerald" />
        <StatItem title="Pending" value={stats.pending} icon={Clock} color="amber" />
        <StatItem title="Avg. Fee" value="0.0001" icon={Briefcase} color="purple" unit="XLM" />
      </div>

      {/* Search & Filter */}
      <div className="flex flex-col lg:flex-row gap-4">
        <div className="relative flex-1">
          <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-blue-300/40" size={18} />
          <input
            type="text"
        ...
        <div className="flex gap-3">
          ...
          >
            ...
            <option value="create_account" className="bg-slate-900">Account Creation</option>
            <option value="change_trust" className="bg-slate-900">Trust Lines</option>
          </select>
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="bg-white/5 border-b border-white/5">
                <th className="px-6 py-5 text-xs font-black text-blue-300/40 uppercase tracking-widest">Transaction</th>
                <th className="px-6 py-5 text-xs font-black text-blue-300/40 uppercase tracking-widest">Asset / Amount</th>
                <th className="px-6 py-5 text-xs font-black text-blue-300/40 uppercase tracking-widest">From / To</th>
                <th className="px-6 py-5 text-xs font-black text-blue-300/40 uppercase tracking-widest">Status</th>
                <th className="px-6 py-5 text-xs font-black text-blue-300/40 uppercase tracking-widest text-right">Date</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-white/5">
              {isLoading
                ...
                : filteredTransactions.map((tx) => (
                    <tr key={tx.id} className="group hover:bg-white/5 transition-all duration-200">
                      <td className="px-6 py-5">
                        <div className="flex items-center gap-4">
                          <div className={`w-11 h-11 rounded-xl flex items-center justify-center border transition-transform group-hover:scale-110 duration-300 ${
                            tx.type === 'payment' ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20'
                            : tx.type === 'create_account' ? 'bg-blue-500/10 text-blue-400 border-blue-500/20'
                            : 'bg-purple-500/10 text-purple-400 border-purple-500/20'
                          }`}>
                            {tx.type === 'payment' ? <ArrowUpRight size={20} /> : <Layers size={20} />}
                          </div>
                          <div>
                        ...
                        {tx.amount ? (
                          <>
                            <div className="text-lg font-black text-white">
                              {tx.amount} <span className="text-blue-400 text-xs font-bold tracking-widest">{tx.assetCode}</span>
                            </div>
                            <div className="text-xs text-blue-300/30 font-medium mt-0.5">Fee: {tx.fee}</div>
                          </>
                        ) : (
                          <span className="text-blue-300/30 font-bold text-xs uppercase">N/A</span>
                        )}
                      </td>
                      <td className="px-6 py-5">
                        <div className="space-y-1.5">
                          <div className="flex items-center gap-2">
                            <span className="text-[10px] font-black bg-white/5 text-blue-300/50 px-1.5 py-0.5 rounded">FR</span>
                            <span className="text-xs font-mono text-blue-200/50">{tx.from.slice(0, 6)}...{tx.from.slice(-6)}</span>
                          </div>
                          {tx.to && (
                            <div className="flex items-center gap-2">
                              <span className="text-[10px] font-black bg-white/5 text-emerald-400/50 px-1.5 py-0.5 rounded">TO</span>
                              <span className="text-xs font-mono text-blue-200/50">{tx.to.slice(0, 6)}...{tx.to.slice(-6)}</span>
                            </div>
                          )}
                        </div>
                      </td>
      ...
      <div className="flex items-center justify-between">
        <p className="text-blue-300/40 font-semibold text-sm">Showing 1–25 of 1,240 results</p>
        <div className="flex gap-2">
          <button className="p-3 bg-white/5 border border-white/10 text-white rounded-xl hover:bg-white/10 transition-all">
            <ChevronLeft size={18} />
          </button>
          {[1, 2, 3].map((n) => (
            <button key={n} className={`w-10 h-10 rounded-xl font-bold text-sm transition-all ${n === 1 ? 'bg-blue-600 text-white' : 'text-blue-300/40 hover:text-white'}`}>
              {n}
            </button>
          ))}
          <button className="p-3 bg-white/5 border border-white/10 text-white rounded-xl hover:bg-white/10 transition-all">
...
function StatItem({ title, value, icon: Icon, color, unit }: StatItemProps) {
  ...
  return (
    <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-6 hover:bg-white/10 transition-all group">
      ...
      <p className="text-blue-300/40 font-bold text-xs uppercase tracking-widest mb-1">{title}</p>
